...
export function EmptyState({ jp, label, hint, cta, ctaLink }: Props) {
  return (
    ...
    >
      <p className="text-4xl font-[var(--br-jp-font)] text-base-content/20 leading-none">
        {jp.charAt(0)}
      </p>
      <p className="text-sm font-[var(--br-jp-font)] text-base-content/50">{jp}</p>
      <p className="font-[var(--br-heading-font)] text-lg font-bold uppercase tracking-tight">
        {label}
      </p>
      <p className="text-xs font-[var(--br-mono-font)] text-neutral max-w-xs">{hint}</p>
      <Link
        to={ctaLink}
